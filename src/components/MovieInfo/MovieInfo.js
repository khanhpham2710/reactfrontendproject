...
function MovieInfo({ movieId }) {
    const { id, title, synopsis, background,
        poster, genre, time, date,
        type, path, rating } = movies[movieId - 1]
        ...
        <Grid container sx={{}}>
            <Grid item xs={12} sm={12} md={4} lg={3} p={4}>
                <Box width="100%" sx={{ width: "100%", height: "100%", display: "flex", justifyContent: "center", alignItems: "center" }}>
                    <img src={poster} alt={title} style={{ width: '100%', height: 'auto', objectFit: 'cover', maxWidth: "250px", border: "#ccc 5px solid", borderRadius: "20px" }} />
                </Box>
            </Grid>
            <Grid item xs={12} sm={12} md={8} lg={9} sx={{ p: { xs: 2, sm: 2, md: 3, lg: 4 } }} textAlign="left">
                <Typography variant='h3' fontWeight="800" gutterBottom fontSize={fontSize2}>
                    {title}
                </Typography>
